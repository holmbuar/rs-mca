...
The results cited in \cite{Cho26CapV13,Cho26Grande} supply the proved
cellwise constructions and estimates in \textup{(C1)--(C8)} and the
Fourier-flat part of \textup{(C9)}.  For a given smooth or circle row they assemble to
...
Recent C9 support packets prove several restricted payments and route cuts:
block-profile Plotkin bounds, near-norm-gate remainder packing,
coherent-phase Fourier subblock payment, and exclusions for radial-shell and
low-complexity cube-parametrized routes \cite{DannyC9Packets}.  These packets
clarify the residual target but do not by themselves exhaust \textup{(C9)} or
replace the Sidon payment required in \cref{def:sidon-paid}.
...
\begin{remark}[Audit status]\label{rem:audit-status}
A line-by-line audit of the profile-envelope draft reports no problem with
the obstruction construction, the natural-profile payment scale, or the
separation between support statistics and ray counting \cite{BuarProfileAudit}.
It leaves as explicit proof obligations the imported window-uniformity input
and the residual higher-dimensional split-pencil charts not reduced to the
paid one-parameter, deficiency-one, or tangent-borne cases.  The deployed-row
numerical spine has also been checked independently as arithmetic support for
the displayed profile-envelope crossing \cite{LegaProfileNumerics}.  Separate
finite threshold packets for the KoalaBear and Mersenne rows sharpen current
finite walls, including the star3 point-count and signed-cancellation target,
but they are not used as hypotheses in the asymptotic theorem
\cite{BuarStar3Packets}.
\end{remark}
...
\bibitem[Bua26a]{BuarProfileAudit}
H. Buar,
\newblock \emph{Asymptotic RS--MCA profile-envelope draft:
line-by-line audit},
\newblock experimental audit note, July 2026.

\bibitem[Bua26b]{BuarStar3Packets}
H. Buar,
\newblock \emph{CAP25 v13 star3 point-count, second-moment, and
threshold-wall packets},
\newblock experimental threshold notes, July 2026.
...
\bibitem[Dan26]{DannyC9Packets}
D. Experiments,
\newblock \emph{C9 endpoint support packets: block-profile Plotkin bounds,
near-norm-gate remainders, radial-shell route cut, cube parametrization
exclusion, and coherent-phase Fourier block},
\newblock experimental threshold notes, July 2026.
...
\bibitem[Leg26]{LegaProfileNumerics}
LegaSage,
\newblock \emph{Profile-envelope quantitative spine at the four deployed
rows},
\newblock experimental audit note, July 2026.
