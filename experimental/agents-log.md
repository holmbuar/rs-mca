...
- **STATUS:** PROVED
- **What is being added:** Proves the abstract finite-set theorem `ROUTE_D_COMPLEMENTARY_CELL_CHARGE`: separately support-injective generated and primitive charges with disjoint images in one `Fin t x F_p` universe give the exact combined `t*p` bound.  An arbitrary complementary cell set is the minimal interface; scalar-coordinate separation remains a stronger corollary alongside complete-base/realized-slot, image-only, local-key, and strict row-separated fixtures sharpening the obstruction at commit `b23f997474f7a7aec9a889d933c774acc4980050`.
- **How it is useful:** Supplies a type-correct sufficient frame for the primitive support target at commit `e83962ae5ad7bacb391b691ffd37f0abef977b83` without imposing an unnecessary scalar-only split, while showing that the generated image ledger at commit `0955594bf354b6a396574b65fbb242715edd3267` remains image-only.  The theorem is proved abstractly; its deployed hypotheses remain open.
- **What to do next:** After the exact named executor missing at commit `8cb3b3ae4c57cf44ef13cda24e4532b3dbe1bb67` exists, construct one target-dependent complementary cell set and support-level charges retaining literal `G`; require scalar separation only when separately proved, and route any actual all-maximal-minors-vanishing family only through the typed owner at commit `a6a3be8b1a967bbec5fa17fc9afa7daaf5b2d0c0`.
