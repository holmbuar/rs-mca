...
- **Agent/model:** Codex.
- **Files added or changed:** `experimental/lean/grande_finale/GrandeFinale/PrefixAtlasBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/FirstMatchWitnessBridge.lean`, `experimental/lean/grande_finale/.gitignore`, `experimental/lean/grande_finale/PREFIX_ATLAS_BRIDGE_CORRESPONDENCE.md`, `experimental/notes/audits/atlas_payment_interface_audit.md`, `experimental/scripts/verify_atlas_payment_interface.py`, and `experimental/data/certificates/atlas-payment-interface/atlas_payment_interface.json`.
- **Status:** AUDIT. Generic prefix-fibre witness totality, the hypothesis-parametric witness-exhaustive/slope-first-match fixed-row `B_MCA` implication, concrete locator-prefix bad-slope unions, and the direct RS fixed-line/outer-line lifts are theorem-level. No actual C1--C9 witness cells, concrete RS witness adapter, cellwise payment, or asymptotic `(UNIF)` is supplied.
- **What is being added:** A typed bridge from locator coefficient fibres to the actual RS MCA bad-slope numerator, together with a generic witness-to-slope first-match bridge. The generic leaf defines slope-level first-match and residual witness cells, proves their exact image equality and guarded supremum/uniform `B_MCA` implications, and includes exact `Fin 2 -> Fin 1` countermodels for slope cover and residual witness non-exhaustivity. The audit separates unconditional coverage from the explicit catalogue, payment, and uniformity hypotheses.
...
- **What to do next:** Construct the actual C1--C9 witness cells and prove `hbadImage` and `hexhaust`, then supply line-dependent first-match slope budgets `hcell` and the uniform line-sum bound `hunif`. Do not infer residual witness exhaustivity, a profile census, or numerical payment from slope or prefix coverage alone.
