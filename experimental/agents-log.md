...
- **Agent/model:** Codex.
- **Files added or changed:** `experimental/lean/grande_finale/GrandeFinale/PrefixAtlasBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/FirstMatchWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardWitnessBridge.lean`, `experimental/lean/grande_finale/.gitignore`, `experimental/lean/grande_finale/PREFIX_ATLAS_BRIDGE_CORRESPONDENCE.md`, `experimental/notes/audits/atlas_payment_interface_audit.md`, `experimental/scripts/verify_atlas_payment_interface.py`, and `experimental/data/certificates/atlas-payment-interface/atlas_payment_interface.json`.
- **Status:** AUDIT. Generic prefix-fibre witness totality, the generic witness-exhaustive/slope-first-match `B_MCA` implication, the finite exact-cardinality RS witness catalogue and its exact bad-slope image, concrete locator-prefix bad-slope unions, and the direct fixed-line/outer-line lifts are theorem-level. No actual C1--C9 witness cells, raw-witness cell exhaustivity, cellwise payment, or asymptotic `(UNIF)` is supplied.
- **What is being added:** Typed locator-prefix and generic witness-to-slope first-match bridges, plus a concrete finite RS `(gamma,S,h)` adapter. The adapter represents `h` by its `Fin k -> F` coefficient vector, proves explainer uniqueness for fixed slope/support, identifies its catalogue's slope image with the threshold-`a` RS MCA-bad slopes, and specializes the guarded supremum/uniform `B_MCA` implications. Exact `Fin 2 -> Fin 1` controls preserve the distinction between slope cover and raw witness exhaustivity.
...
- **What to do next:** Construct the actual C1--C9 witness cells and prove raw-witness `hexhaust`, then supply line-dependent first-match slope budgets `hcell` and the uniform line-sum bound `hunif`. The concrete adapter now discharges `hbadImage`; do not infer the remaining classification, census, or payment from slope or prefix coverage alone.
