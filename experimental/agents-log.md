...
- **Status:** AUDIT. Generic prefix-fibre totality, the generic witness/slope-first-match implication, the finite exact-cardinality RS catalogue and bad-slope image, locator-prefix support and raw-witness exhaustivity, injective-evaluation/parity-conditional exact per-cell slope alignment, the exact explanation-state fibre sum, and the conditional post-slope-first-match RC1 quotient/`B_MCA` adapter are theorem-level. No actual C1--C9 semantic witness classification or payment, intended-profile lower retained-support occupancy, or asymptotic `(UNIF)` is supplied.
- **What is being added:** Typed locator-prefix and generic witness-to-slope first-match bridges, a concrete finite RS `(gamma,S,h)` adapter, its locator-prefix witness lift, and the two-stage projection `(gamma,S,h) -> (gamma,h) -> gamma`. Literal witness cells split exactly over realized explanation-state fibres; if every realized residual state has occupancy at least `H > 0`, its slope cell has size at most `floor(|residual| / H)`, and the resulting line-dependent quotient budgets feed the guarded `B_MCA` implications.
- **How it is useful:** It replaces a stale hard-input shorthand by the exact residual: C3 planted census, intended-profile C7 positive retained-support occupancy/profile collapse (or a direct final-slope estimate), C8 higher-dimensional ray compilation, and C9 Sidon payment. It also records that RC1 is applied after slope first match and does not itself solve semantic C7.
- **What to do next:** Construct the actual C1--C9 witness classifier and its relationship to the structural prefix partition, then prove per-profile positive lower occupancies `hocc` (or direct slope bounds) and the uniform quotient-sum bound `hunif`. The concrete adapters now discharge `hbadImage` and raw prefix-cell `hexhaust`; the occupancy adapter discharges `hcell` only conditional on those occupancy hypotheses. Do not infer semantic classification, census, or payment from structural coverage.
