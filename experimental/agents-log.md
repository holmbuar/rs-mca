...
- **Agent/model:** Codex.
- **Files added or changed:** `experimental/lean/grande_finale/GrandeFinale/PrefixAtlasBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/FirstMatchWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardPrefixWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardOccupancyBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardBoundaryPaymentBridge.lean`, `experimental/lean/grande_finale/.gitignore`, `experimental/lean/grande_finale/PREFIX_ATLAS_BRIDGE_CORRESPONDENCE.md`, `experimental/notes/audits/atlas_payment_interface_audit.md`, `experimental/scripts/verify_atlas_payment_interface.py`, and `experimental/data/certificates/atlas-payment-interface/atlas_payment_interface.json`.
- **Status:** PROVED / CONDITIONAL / AUDIT, depending on the claim. Generic prefix-fibre totality, the generic witness/slope-first-match implication, the finite exact-cardinality RS catalogue and bad-slope image, locator-prefix support and raw-witness exhaustivity, injective-evaluation/parity-conditional exact per-cell slope alignment, the exact explanation-state fibre sum, the conditional post-slope-first-match RC1 quotient/`B_MCA` adapter, the full/residual boundary-fibre identities, and the actual-cell FC1 ray/moment implication are theorem-level. No actual C1--C9 semantic witness classification or payment, intended-profile lower retained-support occupancy, semantic C8 incidence, deployed C9 moment, or asymptotic `(UNIF)` is supplied.
- **What is being added:** Typed locator-prefix and generic witness-to-slope first-match bridges, a concrete finite RS `(gamma,S,h)` adapter, its locator-prefix witness lift, the two-stage projection `(gamma,S,h) -> (gamma,h) -> gamma`, and a full-vs-residual boundary profile for the exact FC1 compiler. Literal witness cells split exactly over realized explanation-state fibres; positive occupancy gives the C7-style quotient. Retained boundary fibres sum exactly to the residual support count, their equal-boundary pairs give the exact second moment, and supplied C8 incidence degrees plus C9 boundary-moment data give the FC1 floor on the actual assigned slope cell.
- **How it is useful:** It replaces a stale hard-input shorthand by the exact residual. The integrated C3 packet proves only the candidate census for explicit subgroup-coset/multiplier-fixed loci; general row-level C3 identification, profile scale, and slope projection remain open. C7 still needs semantic classification plus positive retained-support occupancy (or a direct bound), C8 needs the semantic ray incidence, and C9 needs primitive survival and the deployed image-normalized moment. The adapters now make both the C7 and C8/C9 conclusions exact once those inputs are supplied.
- **What to do next:** Construct the actual C1--C9 witness classifier and its relationship to the structural prefix partition, then prove row-level C3 family/payment, per-profile C7 lower occupancies `hocc` (or direct slope bounds), C8 incidence degrees, C9 residual moments, and the uniform sum `hunif`. The concrete adapters discharge `hbadImage`, raw prefix-cell `hexhaust`, the finite fibre sums, and the FC1 arithmetic only; do not infer semantic classification, census completion, or payment from structural coverage.
