...
### 2026-07-21 - Loss-one C9 prefix key on the deployed M31 domain

- **Agent/model:** GPT-5.6 Pro.
- **Files added or changed:** `experimental/notes/thresholds/sidon_effective_image_mi_ma_c9_rowsharp.md`; `experimental/notes/audits/sidon_effective_image_c9_rowsharp_preflight.md`; replacement stdlib-only package `experimental/lean/sidon_effective_image/`; this entry side-file.
- **Status:** PROVED SCOPED ONE-KEY / CONDITIONAL_ON_NAMED_INPUT / KERNEL_VALIDATED.
- **What is being added:** Eight explicit roots of `T_(2^21)` over `F_(2^31-1)` form two complete `T_4` blocks.  On the complete four-subset slice, the first three power sums have one doubled key formed by complete `T_4` blocks; deleting the six exact C1 antipodal-quotient supports leaves an injective residual.  One quotient-free key has singleton full and residual fibers, giving the literal producer field `fullPrefixFiber.length <= 1*2` with loss factor one; any genuine SE2 cell on that key receives a direct C9 payment.
- **How it is useful:** This is a zero-slack deployed-domain instantiation of the row-sharp field targeted by `def:q-row-atom` / `prop:q-exact-target`, with owner-complement and slope-projection boundaries kept explicit.  It does not claim all-key coverage or row closure.
- **What to do next:** Prove `M31_C9_GLOBAL_OWNER_COMPLEMENT_AND_KEY_COVERAGE`: construct the actual deployed C1--C8 owner function, cover every surviving prefix key by loss-one bounds or an exact counterexample, and retain SE2, profile count, add-back, and line-local UNIF.
...
- **Files added or changed:** Added `experimental/notes/thresholds/affine_prefix_fixed_point_half_slice.md`, `experimental/scripts/verify_affine_prefix_fixed_point_half_slice.py`, and its certificate folder; added `experimental/notes/thresholds/affine_prefix_pole_stratum.md`, `experimental/scripts/verify_affine_prefix_pole_stratum.py`, and its certificate folder; added `experimental/notes/thresholds/primitive_top_seam_marked_incidence_v1.md`, `experimental/scripts/verify_primitive_top_seam_marked_incidence_v1.py`, and its JSON certificate folder; added the Lean package `experimental/lean/complete_fiber_subset_packing/`, `experimental/notes/l2/complete_fiber_subset_packing_formalization.md`, and `experimental/scripts/verify_complete_fiber_subset_packing_formalization.py`.  Contributor `experimental/agents-log.md` edits from the PR branches were not imported.
- **Status:** PROVED_LOCAL / PROVED / COUNTEREXAMPLE / AUDIT / OPEN_GAP / EXPERIMENTAL, according to the individual notes.
- **What is being added:** #976 isolates the exact fixed-point half of the characteristic-five affine-prefix `j=0` stratum and records the owner-or-exponent inequality without claiming semantic survival.  #979 proves a characteristic-five whole-stratum pole lower bound for a line-local family, again as a structural route cut rather than a semantic first-match payment.  #978 audits the primitive top-seam marked-incidence interface, proves finite marked-count regressions, and refutes selected-support/unique-mate/common-core shortcuts.  #977 formalizes the complete-fiber subset-packing compiler in Lean, deriving finiteness and the exact floored binomial quotient used by the L2 equation-(2) wrapper.
- **How it is useful:** The two affine-prefix packets are useful stress tests for the first-match atlas and source-versus-slope projection discipline.  The top-seam audit is immediately useful as a regression against overcounting marked triples or erasing common-core multiplicity before slope projection.  The Lean package is the most directly reusable item: it gives a theorem-level formal combinatorial compiler for complete-fiber subset packing, while explicitly leaving normalization, deployed arithmetic, and Grand MCA/List conclusions outside scope.  None of these PRs adds a new leaderboard row or promotes a main-paper theorem.
- **What to do next:** Audit #976/#979 against the actual C1--C9 first-match owner order before using them in any safe-side budget.  Use #978 as a required regression whenever a top-seam/SP argument projects marked incidences to MCA slopes.  For #977, do not call the L2 route Lean-certified until the package is built in a dedicated Lean environment and the theorem statement is matched to the current TeX node; the group-cardinality rewrite and equation-(3) arithmetic remain open.
...
- **Agent/model:** Codex, integrating reviewed PRs from Holm Buar,
  DannyExperiments, Scott Hughes, and Latif Kasuli.
...
- **What is being added:** A reviewed experimental wave covering Route-D
  owner-typed forest and marked-recursion formalizations, M1 KoalaBear
  source--Mobius and projective-base owner refinements, L2 rank-16 fixed-26
...
- **How it is useful:** The most useful mathematical movement is #962, which
  ...
  `U_A`, and lower-gcd routes open.  The most useful audit movement is #965,
  which keeps the dense-shell Arb transfer certificate in OPEN-GAP status
...
- **What to do next:** Audit #962 against the current M1/KoalaBear safe-side
  ledger before any paper promotion, replay or replace the stale Arb evidence
