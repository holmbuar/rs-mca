...
### 2026-07-18 - Route-D Rule-2 WSP algebraic preflight

- **Agent/model:** Codex.
- **Files added or changed:** `experimental/notes/thresholds/route_d_rule2_wsp_algebraic_preflight_v1.md`, `experimental/scripts/verify_route_d_rule2_wsp_algebraic_preflight_v1.py`, and the standalone Lean package `experimental/lean/route_d_rule2_wsp_algebraic_preflight_v1/`.
- **STATUS:** COUNTEREXAMPLE
- **What is being added:** An exact `F_31` algebraic replay of the top-seam Rule-2 construction from predecessor commit `a6a3be8b1a967bbec5fa17fc9afa7daaf5b2d0c0`.  It enumerates `121` mates and `119` projectively primitive packets, checks the `28`-cell/`91`-emission decomposition, records the primary `6/85` and auxiliary `3/88` extension splits, and proves the order-invariant lower count of `81` nonextension projectively primitive algebraic candidates.
- **How it is useful:** It refutes treating Rule-2 certificate shape and exact support cost one as a numerical payment.  The packet preserves the common-core mark while keeping the candidate census explicitly before branch-excess, canonical-packet, prior-filter, owner, and payment interfaces.
- **What to do next:** Construct genuine branch-excess units and canonical `Pi(B,C)` packets, execute `P1` and every prior first-match predicate, and supply a typed accepting-owner theorem before using any algebraic candidate count in a safe-side ledger.
