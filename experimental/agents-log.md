...
- **Agent/model:** Codex.
- **Files added or changed:** `experimental/lean/grande_finale/GrandeFinale/PrefixAtlasBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/FirstMatchWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardWitnessBridge.lean`, `experimental/lean/grande_finale/GrandeFinale/RSExactCardPrefixWitnessBridge.lean`, `experimental/lean/grande_finale/.gitignore`, `experimental/lean/grande_finale/PREFIX_ATLAS_BRIDGE_CORRESPONDENCE.md`, `experimental/notes/audits/atlas_payment_interface_audit.md`, `experimental/scripts/verify_atlas_payment_interface.py`, and `experimental/data/certificates/atlas-payment-interface/atlas_payment_interface.json`.
- **Status:** AUDIT. Generic prefix-fibre totality, the generic witness/slope-first-match implication, the finite exact-cardinality RS catalogue and bad-slope image, locator-prefix support and raw-witness exhaustivity, injective-evaluation/parity-conditional exact per-cell slope alignment, and the direct fixed-line/outer-line lifts are theorem-level. No actual C1--C9 semantic witness classification, cellwise payment, or asymptotic `(UNIF)` is supplied.
- **What is being added:** Typed locator-prefix and generic witness-to-slope first-match bridges, a concrete finite RS `(gamma,S,h)` adapter, and its locator-prefix witness lift. The lift partitions the literal catalogue by the selected support's prefix, proves raw `hexhaust`, identifies each cell's slope image with the existing support-prefix bad-slope cell under injective evaluation and the parity-dimension equation, and specializes the guarded `B_MCA` implications. Exact controls preserve the distinction between structural prefix coverage and semantic C1--C9 classification/payment.
- **How it is useful:** It replaces a stale hard-input shorthand by the exact residual: C3 planted census, C7 projection degree, C8 higher-dimensional ray compilation, and C9 Sidon payment. It also records that first match is performed after slope projection and that the Sidon-paired consumer still needs primitive survival after C1--C8 on the deployed row or an earlier classification/payment.
- **What to do next:** Construct the actual C1--C9 witness classifier and its relationship to the structural prefix partition, then supply line-dependent first-match slope budgets `hcell` and the uniform line-sum bound `hunif`. The concrete prefix adapter now discharges `hbadImage` and raw prefix-cell `hexhaust`; do not infer semantic classification, census, or payment from that structural coverage.
