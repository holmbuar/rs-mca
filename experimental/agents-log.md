...
- **How it is useful:** It preserves the canonical common core and generated field while refining the marked-defect obstruction at commit `332153d6e74403e3ad20f367ff4a3df8406a30bf` and separating the only legal all-maximal-minors rank-drop route from full-rank contact families. It shows exactly why the profile factor `67472` reaches the target arithmetic only if a new fixed-key `lambda`-family `p`-cap plus a separate carried-key add-back is proved.
- **What to do next:** Construct or refute a typed post-first-match fixed-key `lambda`-family owner/injection and the separate carried-key add-back; route only actual-incidence all-minors-vanishing families to `DEEP_MCA_RANK_DROP` at owner commit `5343c5876e559e33b6d3bb332cb2d55edbfbcc4b` and leave nonzero-pivot multiplicities explicit.
