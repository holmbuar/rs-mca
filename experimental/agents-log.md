...
### 2026-07-18 - Profile-envelope target correspondence audit

- **Agent/model:** Codex.
- **Files added or changed:** Added `experimental/notes/audits/profile_envelope_target_correspondence_audit.md` @ `10620fcd` and `experimental/scripts/verify_profile_envelope_target_correspondence_audit.py` @ `624606e3`; repaired `experimental/notes/thresholds/profile_envelope_target_comparison.md` @ `50ccfaf7`, `experimental/scripts/verify_profile_envelope_target_comparison.py` @ `49d39d56`, `experimental/data/certificates/profile-envelope-target-comparison/cert.json` @ `2b801eb0` (SHA-256 `452f2fdc8ee5d98153f7afffce33b3cddcdf05ada422b9d504daa423198ba408`), `experimental/lean/profile_envelope_target_comparison/ProfileEnvelopeTargetComparison.lean` @ `a68372d6`, `experimental/notes/thresholds/nonfiber_decomposition_realized_scale.md` @ `86c7634b`, `experimental/scripts/verify_nonfiber_decomposition_realized_scale.py` @ `857c68e3`, and `experimental/lean/nonfiber_decomposition_realized_scale/NonfiberDecompositionRealizedScale.lean` @ `04cc425d`; updated this promotion entry.
- **Status:** COUNTEREXAMPLE
- **What is being added:** An independent audit of integration `2633895a` (= PR #759 head `8cd4f4b6`) records a `COUNTEREXAMPLE` to the exact universal claim that prime base, no field drop, and full identity image force identity dominance and a complete envelope: the claim is the `(c-i) DECIDE (prime rows)` / `Headline` / section 3 `(c-i)` / ledger `C3` interface in `experimental/notes/thresholds/profile_envelope_target_comparison.md:36-40,57-62,243-255,300-308` @ `436f5c95`, while verifier functions `section_B_prime` and `section_F_reduction` at `experimental/scripts/verify_profile_envelope_target_comparison.py:424-446,622-650` @ `81ac80d0` accept identity **or** the deep term and then print identity/whole-envelope dominance. The packet's own GF(13) `c=3,r=0` cell has `6>924/169`, and the independent no-drop GF(19) row has full identity image `19^3` but `126/19>43758/6859`; the repair also retracts the exact-`(FI)`, unsafe-target, and certificate overclaims in `experimental/data/certificates/profile-envelope-target-comparison/cert.json:10-75` @ `f1734d43` plus the Lean name `tower121_identity_FI_fails` at `experimental/lean/profile_envelope_target_comparison/ProfileEnvelopeTargetComparison.lean:13-25,89-100` @ `4cb72278`, without changing submission-facing files.
- **How it is useful:** It preserves the selected finite arithmetic while restoring the received-line/first-match and one-way-safe interfaces in `experimental/asymptotic_rs_mca_frontiers.tex:843-862,1452-1472,6154-6161` @ `466b35c5` and the open upper-payment boundary in `experimental/notes/thresholds/profile_envelope_completeness.md:145-172,248-261` @ `6f56feca`. Producer interlocks remain PR #520 head `73525004`, PR #524 head `5fe12fba`, PR #542 head `dcda4e9f`, PR #606 head `96dc5370`, and PR #688 head `48b6487d`; PR #713 head `d78ea57e` and PR #714 head `1df8a072` remain adjacent unpaid ledgers, while same-wave consumer PR #760 head `dabf6510` is repaired as motivational and proof-independent.
- **What to do next:** Run `python3 experimental/scripts/verify_profile_envelope_target_correspondence_audit.py` (verifier `experimental/scripts/verify_profile_envelope_target_correspondence_audit.py` @ `624606e3`) -> `RESULT: PASS (21/21)`, `STATUS: COUNTEREXAMPLE`, identically under `python3 -O`; its normal/optimized tamper self-tests reject `3/3`. Run `python3 experimental/scripts/verify_profile_envelope_target_comparison.py` (producer `experimental/scripts/verify_profile_envelope_target_comparison.py` @ `49d39d56`) -> `RESULT: PASS (89/89)`, `STATUS: COUNTEREXAMPLE`, and `python3 experimental/scripts/verify_nonfiber_decomposition_realized_scale.py` (consumer `experimental/scripts/verify_nonfiber_decomposition_realized_scale.py` @ `857c68e3`) -> `RESULT: PASS (90/90)`; direct compilation of `experimental/lean/profile_envelope_target_comparison/ProfileEnvelopeTargetComparison.lean` @ `a68372d6` and `experimental/lean/nonfiber_decomposition_realized_scale/NonfiberDecompositionRealizedScale.lean` @ `04cc425d` must exit `0`. Keep the result scoped to selected profiles and correspondence repair; it proves no complete envelope, asymptotic `(FI)` verdict, unsafe row, threshold, or submission theorem.
