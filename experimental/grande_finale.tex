...
The labels Q, BC, and SP are retained only as names for residual safe-side mechanisms.  The section records actual theorems rather than certificate compilers.  The proved Q bound below is an unconditional packing ceiling, far weaker than the desired max-fiber theorem.  The proved BC statements distinguish raw support locators from the saturated codeword rays that MCA actually counts.  The proved SP statement has two forms: a crude all-pairs ceiling, and the sharper structural fact that SP is automatically controlled once Q is available as a max-fiber bound.  None of these statements is advertised as closing the dense frontier.
...
\item The residual cells have unconditional theorem-level pieces: Q has the Johnson packing and anticode bounds of \cref{thm:q-proper}; raw split-pencil support counts have the dimension bound of \cref{thm:bc-proper}; MCA support multiplicities are exactly saturated by \cref{thm:saturation,cor:raw-bc-fails,prop:line-ray-saturation}; and SP is downstream from Q by \cref{thm:q-implies-sp}, with the all-pairs ceiling of \cref{thm:sp-proper} available as an audit bound.
\item The stronger dense-frontier conclusion is not proved by these bounds.  In particular, \cref{prop:proper-q-gap} shows that the proved Q packing bound alone leaves overhead far beyond the active adjacent margins, and \cref{cor:raw-bc-fails} shows that the MCA split-pencil target must be saturated before it can be an MCA numerator.
\end{enumerate}
\end{theorem}

\begin{proof}
Parts (i) and (ii) are the cited theorems.  Part (iii) is exactly \cref{thm:q-proper,thm:bc-proper,thm:saturation,cor:raw-bc-fails,prop:line-ray-saturation,thm:q-implies-sp,thm:sp-proper}.  Part (iv) follows from \cref{prop:proper-q-gap}: an upper estimate that permits more than \(2^{1.66\cdot10^6}\) normalized prefix overhead cannot imply a ledger with only \(2^{22.2}\) or less spare overhead.  The saturation identity also changes the MCA split-pencil target from raw supports to primitive line-rays.  Therefore the adjacent safe inequality requires stronger row-wise arguments than the unconditional replacement bounds proved here.
\end{proof}
...
\item The unconditional Q packing bound, raw split-pencil dimension bound, saturation identity, and SP-from-Q theorem of \cref{thm:q-proper,thm:bc-proper,thm:saturation,thm:q-implies-sp} are proved, but they do not by themselves imply \(B(a_0+1)\le B^*\) for any of the four adjacent rows.
\end{enumerate}
\end{theorem}

\begin{proof}
Part (i) and the lower-route stopping statement in part (ii) are \cref{prop:finite-packet-consequences}.  Part (iii) is \cref{prop:finite-safe-anchor-gap}.  Part (iv) follows because the first residual replacement bound already has the gap quantified in \cref{prop:proper-q-gap}; the saturation theorem changes the object to be counted but does not supply the needed saturated primitive line-ray bound; and \cref{thm:q-implies-sp} only discharges SP after a sufficiently sharp Q max-fiber theorem has been proved.  Hence none of the proved statements supplies the missing inequality \(B(a_0+1)\le B^*\).
\end{proof}
...
\item The Q, BC, and SP replacements in \cref{thm:q-proper,thm:bc-proper,thm:saturation,prop:line-ray-saturation,thm:q-implies-sp,thm:sp-proper} are unconditional statements, but their numerical strength is insufficient for the four active adjacent agreements.
\item No statement in this note proves \(B(a_0+1)\le B^*\) for any deployed adjacent row.
\end{enumerate}
\end{proposition}

\begin{proof}
Part (i) is \cref{prop:prefix-witness,thm:simple-pole-list-floor,cor:identity-prefix-unsafe,thm:unsafe-envelope}.  Part (ii) is \cref{thm:gf-deep-mca,prop:exact-tangent-cell,thm:gf-mca-from-ca,thm:subfield-confinement,prop:finite-safe-anchor-gap}.  Part (iii) follows from the cited residual-cell theorems and from \cref{prop:proper-q-gap}.  Part (iv) is then immediate from \cref{thm:deployed-row-status}: the only theorem in this note that would turn the banked unsafe edge into a closed adjacent threshold is \cref{thm:finite}, and its upper-bound hypothesis is not supplied by the proved replacement bounds.
\end{proof}
...
\begin{problem}[saturated primitive split-pencil line-ray target]\label{prob:saturated-bc}
For each active MCA adjacent row, after quotient, boundary-Q, common-support, tangent, and extension branches have been paid by first match, bound the number of residual bad slopes through saturated primitive line rays:
\[
        |\{z:\exists c\ (z,c)\in\LineRay_E(u,v;a_+)\text{ and }z\text{ is residual-bad}\}|.
\]
By \cref{prop:line-ray-saturation}, a raw support-locator census is only an upper audit object,
\[
        \sum_{z\in E}\Cen(u+zv;a_+)
        =
        \sum_{(z,c)\in\LineRay_E(u,v;a_+)}\binom{s_{z,c}}{a_+},
\]
and can be exponentially larger than the slope numerator.  The required theorem must therefore control either the distinct slope set directly or the saturated line-ray set with a proved multiplicity loss small enough for the exact row margin.
\end{problem}

\begin{remark}[finite arithmetic artifacts]\label{rem:finite-arithmetic-artifacts}
The exact integer tables in this note can be checked independently from the displayed formulas using integer arithmetic only: \(B^*=\lfloor p^s2^{-\lambda}\rfloor\), \(L(a)=\lceil\binom na p^{-(a-K)}\rceil\), the simple-pole value \(M(a)=\lceil L(a)(q-n)/(q-n+k(L(a)-1))\rceil\), the extension headroom in \cref{prop:extension-cell-target}, and the logarithmic Q-packing estimate in \cref{prop:proper-q-gap}.  These checks are arithmetic audits; they do not replace the missing upper-ledger theorems in \cref{prob:row-sharp-q,prob:saturated-bc}.
\end{remark}

\section{Remaining Finite Upper-Ledger Target}

The proper proofs above remove the fake certificate-theorem closure.  They prove real replacement statements, but they do not settle the dense frontier.  A finite adjacent proof must improve the upper bound \(U(a_0+1)\) in \cref{thm:finite} until it fits inside the exact margins \(22.1969,22.0109,3.2589,3.0730\) bits.  After \cref{thm:q-implies-sp}, the independent active MCA targets are multilevel Q and saturated primitive BC; SP remains an exact audit ledger and follows once Q is strong enough.
...
\paragraph{Split-pencil interior ledger.}  The split-pencil proof must count saturated primitive line-rays, not raw support locators.  The exact saturation identity \cref{thm:saturation} explains the difference, and \cref{cor:raw-bc-fails} shows why the raw count can be exponentially larger than the MCA numerator.  The remaining proof must improve the dimension bound of \cref{thm:bc-proper} by using the actual split condition, quotient removal, base-field normalization, saturation, and row-specific chart equations.  The base-field floor in \cref{prop:base-field-floor} shows that challenge-field normalization alone is false.
...
This version contains no Q, BC, or SP certificate theorem.  The unsafe side is exact and the broad safe anchors are unconditional in their stated ranges.  The former SP input is now a theorem downstream from Q; the raw BC support count is replaced by exact saturation and line-ray identities; and the unconditional Q/BC/SP ceilings that remain are explicitly proved but too weak for the deployed adjacent rows.  The full entropy--subfield equality and the adjacent safe inequalities are therefore not asserted.  A closed adjacent theorem still requires a row-sharp multilevel Q max-fiber bound and a saturated primitive split-pencil line-ray bound satisfying \cref{thm:finite}.
