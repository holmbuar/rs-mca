...
`hashes.json` pins every packet artifact except itself.  After any edit,
regenerate all hashes and rerun both verifier modes.  The verifier checks giant
binomial arithmetic by two independent routes.  Lean validation is stdlib-only
and is performed by fork CI; no local Lean build is authoritative.
