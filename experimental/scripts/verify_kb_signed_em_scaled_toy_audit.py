...
def data_gate(data):
    try:
        deployed = data["deployed_kb_mca_reference"]
        toy = data["scaled_toy"]
        modes = data["mode_classification"]
        scope = data["scope_limits"]
        expected_concentration = [
            ...
            for frac, o1, q1, o2, q2 in concentrations
        ]
        return (
            data["schema"] == "cap25-v13-kb-signed-em-scaled-toy-audit-v1"
            and data["status"] == STATUS
            and deployed["p"] == DEPLOY_P
            and deployed["n"] == DEPLOY_N
            and deployed["k"] == DEPLOY_K
            and deployed["a_plus"] == DEPLOY_A
            and deployed["m"] == DEPLOY_M
            and deployed["w"] == DEPLOY_W
            and deployed["Bstar"] == DEPLOY_BSTAR
            and deployed["t_times_p_reservation"] == DEPLOY_TP
            and deployed["Brem"] == DEPLOY_BREM
            and deployed["avg_floor"] == DEPLOY_AVG_FLOOR
            and deployed["avg_ceil"] == DEPLOY_AVG_CEIL
            and deployed["Kraw_floor"] == DEPLOY_KRAW
            and deployed["Krem"] == DEPLOY_KREM
            and deployed["target_floor"] == DEPLOY_TARGET_FLOOR
            and deployed["nu_ref"] == DEPLOY_NU_REF
            and deployed["primitive_orbit_budget_num"] == 5772020665009
            and deployed["primitive_orbit_budget_den"] == 524288
            and math.isclose(deployed["primitive_orbit_budget"], float(DEPLOY_PRIMITIVE_ORBIT_BUDGET), abs_tol=5e-9)
            and math.isclose(deployed["primitive_orbit_budget_log2"], math.log2(float(DEPLOY_PRIMITIVE_ORBIT_BUDGET)), abs_tol=5e-12)
            and toy["p"] == P
            and toy["n"] == N
            and toy["m"] == M
            and toy["w"] == W
            and toy["subgroup_index"] == (P - 1) // N
            and toy["C"] == C
            and toy["frequency_space"] == space
            and toy["nonzero_modes"] == space - 1
            and toy["fiber_min"] == minN
            and toy["fiber_max"] == maxN
            and toy["orbit_count"] == len(orbits)
            and toy["primitive_orbit_count"] == primitive_orbits
            and toy["quotient_orbit_count"] == quotient_orbits
            and math.isclose(toy["avg"], fraction_float(avg), rel_tol=5e-16)
            and math.isclose(toy["log2_avg"], math.log2(fraction_float(avg)), abs_tol=5e-10)
            and math.isclose(toy["Rmax"], fraction_float(Rmax), abs_tol=5e-13)
            and math.isclose(toy["Gamma2_minus_1"], Gamma2_minus_1, rel_tol=5e-12)
            and math.isclose(toy["L1_over_C"], L1 / C, abs_tol=5e-13)
            and math.isclose(toy["PR"], PR, abs_tol=5e-9)
            and math.isclose(toy["normalized_PR"], PR / (space - 1), abs_tol=5e-12)
            and math.isclose(toy["primitive_PR"], PR_prim, abs_tol=5e-9)
            and math.isclose(toy["primitive_orbit_PR"], PR_primitive_orbits, abs_tol=5e-9)
            and math.isclose(toy["primitive_L1_share"], primitive_share, abs_tol=5e-12)
            and math.isclose(toy["triangle_over_true_max"], triangle_factor, abs_tol=5e-12)
            and math.isclose(modes["mixed_primitive"]["L1_share"], mixed_L1_share, abs_tol=5e-12)
            and math.isclose(modes["mixed_primitive"]["L2_share"], mixed_L2_share, abs_tol=5e-12)
            and math.isclose(modes["top_two_mixed_orbits_L2_share"], top_two_L2_share, abs_tol=5e-12)
            and math.isclose(modes["linear"]["L1_share"], spec["by_category"]["linear"][0] / L1, abs_tol=5e-12)
            and math.isclose(modes["linear"]["L2_share"], spec["by_category"]["linear"][1] / L2, abs_tol=5e-12)
            and math.isclose(modes["quadratic_quotient"]["L1_share"], spec["by_category"]["quadratic_quotient"][0] / L1, abs_tol=5e-12)
            and math.isclose(modes["quadratic_quotient"]["L2_share"], spec["by_category"]["quadratic_quotient"][1] / L2, abs_tol=5e-12)
            and data["mass_concentration"] == expected_concentration
            and scope["deployed_subgroup_index"] == (DEPLOY_P - 1) // DEPLOY_N
            and scope["geometry_faithful"] is False
            and scope["finite_space_sanity_is_automatic"] is True
            and len(data["nonclaims"]) >= 5
            and "remains OPEN" in data["verdict"]
        )
...
print("\n== §3 dilation-orbit compressed signed-e_m spectrum ==")
...
print("\n== §4 scaled-toy measurements and falsification guards ==")
...
check("JSON replay data match all gated exact/measured outputs", data_gate(DATA),
      JSON_PATH)
...
bad_data["scaled_toy"]["fiber_max"] -= 1
rejected("json_fiber_max", data_gate(bad_data))
bad_data = json.loads(json.dumps(DATA))
bad_data["status"] = "PROVED_DEPLOYED"
rejected("json_status_promotion", data_gate(bad_data))
