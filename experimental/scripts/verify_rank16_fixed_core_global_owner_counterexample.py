#!/usr/bin/env python3
"""Exact replay for the rank-16 fixed-core global-owner counterexample.

Python standard library only. This verifier reconstructs the integrated #838
baseline and pending #861 ledger, then checks a literal same-word pair that
defeats total fixed-27/fixed-26 ownership and realizes exact top-layer core
incidence multiplicities 28 and 378.
"""
...
ERROR_SIZE = N - M
LOCATOR_GENERATOR_DEGREE = M - K + 1
TOP_RESIDUAL_DEGREE = ERROR_SIZE - 28 * B
ZERO_PROFILE = (0, 0, 0, 0, 0, 0)
...
def multiplicative_order(value: int) -> int:
    order = P - 1
    for prime in (2, 127):
        while order % prime == 0 and pow(value, order // prime, P) == 1:
            order //= prime
    return order


def counterexample_values(values: dict[str, object]) -> dict[str, object]:
    primitive_root = 3
    omega = pow(primitive_root, (P - 1) // N, P)
    zeta = pow(omega, B, P)

    top_counts = (B,) * 28 + (1_767,) * 25 + (1_766,) * 11
    lower_counts = (
        (30_358,) * 20
        + (30_359,) * 8
        + (3_642,) * 8
    ...
    error_intersection = sum(min(a, b) for a, b in zip(top_counts, lower_counts))
    agreement_intersection = N - (2 * ERROR_SIZE - error_intersection)
    one_sided_swap = ERROR_SIZE - error_intersection

    x_rank = {
        profile: rank
        for rank, (_cap, profile, _sets) in enumerate(values["x_rows"], 1)
    }
    lower_johnson = johnson_parameters(0, 0)
    full_labels = tuple(range(28))
    q32_pairs = sum(1 for label in full_labels if label + 32 in full_labels)
    fixed27_degree = TOP_RESIDUAL_DEGREE + B
    fixed27_quotient_degree = fixed27_degree - LOCATOR_GENERATOR_DEGREE
    fixed26_post_core_degree = TOP_RESIDUAL_DEGREE + 2 * B

    return {
        "primitive_root": primitive_root,
        "primitive_order": multiplicative_order(primitive_root),
        "omega": omega,
        "omega_order": multiplicative_order(omega),
        "zeta": zeta,
        "zeta_order": multiplicative_order(zeta),
        "top_counts": top_counts,
        "lower_counts": lower_counts,
        "top_size": sum(top_counts),
        "lower_size": sum(lower_counts),
        "error_intersection": error_intersection,
        "agreement_intersection": agreement_intersection,
        "one_sided_swap": one_sided_swap,
        "top_full_labels": full_labels,
        "q32_pairs": q32_pairs,
        "top_residual": sum(top_counts[28:]),
        "top_footprint": sum(1 for count in top_counts[28:] if count),
        "lower_touched": sum(1 for count in lower_counts if count),
        "lower_full": sum(1 for count in lower_counts if count == B),
        "x_rank": x_rank[ZERO_PROFILE],
        "zero_profile_in_q110": ZERO_PROFILE in values["q110"],
        "zero_profile_in_q41": ZERO_PROFILE in values["q41"],
        "zero_profile_in_x175": ZERO_PROFILE in {
            row[1] for row in values["x_selected"]
        },
        "lower_johnson": lower_johnson,
        "fixed27_cells": comb(28, 27),
        "fixed26_cells": comb(28, 26),
        "generator_degree": LOCATOR_GENERATOR_DEGREE,
        "fixed27_degree": fixed27_degree,
        "fixed27_quotient_degree": fixed27_quotient_degree,
        "fixed26_post_core_degree": fixed26_post_core_degree,
        "full_locator_degree": ERROR_SIZE,
    }
...
def replay(params: ReplayParameters = ReplayParameters()) -> dict[str, object]:
    ...
    values["counterexample"] = counterexample_values(values)
    return values
...
def verify(values: dict[str, object], repository_root: Path) -> None:
    ...
    witness = values["counterexample"]
    require(P - 1 == (1 << 24) * 127, "field factorization")
    require(witness["primitive_order"] == P - 1, "primitive root")
    require(witness["omega_order"] == N, "evaluation subgroup generator")
    require(witness["zeta_order"] == 64, "q64 label generator")
    require(len(witness["top_counts"]) == 64, "top fiber count")
    require(len(witness["lower_counts"]) == 64, "lower fiber count")
    require(witness["top_size"] == ERROR_SIZE, "top error size")
    require(witness["lower_size"] == ERROR_SIZE, "lower error size")
    require(witness["error_intersection"] == 913_633, "error intersection")
    require(witness["agreement_intersection"] == K - 1, "agreement intersection")
    require(witness["one_sided_swap"] == LOCATOR_GENERATOR_DEGREE, "swap size")
    require(witness["top_full_labels"] == tuple(range(28)), "top full labels")
    require(
        not set(witness["top_full_labels"]).intersection(
            {label + 32 for label in witness["top_full_labels"]}
        ),
        "nonpaired top labels",
    )
    require(witness["q32_pairs"] == 0, "zero q32 pairs")
    require(witness["top_residual"] == TOP_RESIDUAL_DEGREE, "top residual degree")
    require(witness["top_footprint"] == 36, "top residual footprint")
    require(witness["lower_touched"] == 64, "lower touches all blocks")
    require(witness["lower_full"] == 0, "lower has no full block")
    require(not witness["zero_profile_in_q110"], "zero profile outside Q110")
    require(not witness["zero_profile_in_q41"], "zero profile outside Q41")
    require(witness["x_rank"] == 501, "zero profile X rank")
    require(not witness["zero_profile_in_x175"], "zero profile outside X175")
    require(witness["lower_johnson"] is None, "lower outside J48 pool")
    require(witness["fixed27_cells"] == 28, "fixed27 incidence degree")
    require(witness["fixed26_cells"] == 378, "fixed26 incidence degree")
    require(witness["generator_degree"] == 67_472, "generator degree")
    require(witness["full_locator_degree"] == ERROR_SIZE, "full locator degree")
    require(witness["fixed27_degree"] == 96_369, "fixed27 reduced degree")
    require(witness["fixed27_quotient_degree"] == 28_897, "fixed27 quotient degree")
    require(witness["fixed26_post_core_degree"] == 129_137, "fixed26 post-core degree")
...
def render(values: dict[str, object]) -> list[str]:
    ...
    quotient, remainder = divmod(int(values["new_allowance"]), 1_641)
    return [
        "RANK16_FIXED_CORE_GLOBAL_OWNER_COUNTEREXAMPLE: PASS",
        f"field=p{P},n{N},B{B},primitive_root:{witness['primitive_root']},omega_order:{witness['omega_order']},zeta_order:{witness['zeta_order']}",
        f"same_received_word=YES,error_intersection:{witness['error_intersection']},agreement_intersection:{witness['agreement_intersection']}=K-1,swap:{witness['one_sided_swap']}=a",
        f"lower_witness=profile{ZERO_PROFILE},f64=0,touched:{witness['lower_touched']},owner=UNPAID_AFTER_861",
        "lower_coverage=fixed27:NO,fixed26:NO,reason=no_complete_error_core",
        f"top_witness=profile{ZERO_PROFILE},f64=28,residual:{witness['top_residual']},footprint:{witness['top_footprint']},q32_pairs:{witness['q32_pairs']},owner=UNPAID_AFTER_861",
        f"top_local_cells=fixed27:{witness['fixed27_cells']},fixed26:{witness['fixed26_cells']},same_g=X^{witness['generator_degree']},same_nonzero_ray=YES",
        f"natural_owner=INCIDENCE_RELATION,projection_multiplicity=fixed27:{witness['fixed27_cells']},fixed26:{witness['fixed26_cells']}",
        "multiplicity_one_selector=NONPAYING_WITHOUT_OCCUPIED_SOURCE_CELL_BOUND",
        f"finite_ledger=residual:{values['new_allowance']},profiles:{len(values['residual_profiles'])},uniform_cap:{quotient},remainder:{remainder},delta:0",
