...
PR_BODY_REL = Path("PR_BODY.md")
LOG_ENTRY_REL = Path("experimental/agents-log-entry-gptpro-kb-uq-boundary-prefix.md")
LOG_PATCH_REL = Path("experimental/agents-log-kb-uq-boundary-prefix.patch")
...
def tamper_selftest(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kb-uq-semantic-tamper-") as tmp:
        ...
        rewrite_hash_for(tmp_root, MANIFEST_REL)
        failed = False
        try:
