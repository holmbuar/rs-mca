...
The module imports the rooted-shell arithmetic and multiplicative route cut
from upstream PR #1005.  It does not repeat either proof or finite support
catalogue.
...
| mandatory F_241 owner regression | `f241_semantic_target_forces_certified_owner` | Every semantic disjunction on the ten-neighbor packet returns an earlier paid owner. |
| all-residual rejection | `f241_all_residual_rejects_semantic_target` | An actual-residual declaration plus the semantic target would imply the false F_241 envelope. |
...
`SemanticEnvelopeOrOwner` is the exact finite interface.  The owner certificate
is stronger than a support tag because its type includes semantic validity,
the entire witness/codeword/ray/slope chain, membership in a deduplicated
line-local slope image, the exact slope numerator and slope denominator, and
the owner's natural prefix-profile scale.

On `IsActualPostC1C8Residual`, the owner branch is contradictory, so
`semantic_three_plus_seven_on_actual_residual` yields the literal local
inequality.  Conversely, a strict violation of the first branch yields the
owner certificate through `violation_forces_certified_earlier_owner`.

## Proof boundary

Lean kernel-checks the types, finite logical compilers, budget propagation, and
F_241 regression.  The following remain hypotheses or future instantiations:
...
- `SemanticEnvelopeOrOwner` for every deployed rooted shell;
...
`M31QRootedShell.lean` imports `SemanticOwner.lean`.  The authoritative build is
the fork draft-PR GitHub Actions run; no local Lean build is claimed.
