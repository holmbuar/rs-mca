...
A direct payment can be numerically correct and still fail to populate a closed
line when the same slope was already assigned to an earlier cell.  The correct
line-local sum retains the C1 payment and gives C7 no assigned slope.
-/
...
/-- The raw C7 payment satisfies the direct natural-scale inequality; the
obstruction below is ownership, not numerical payment. -/
...
/-- Installing both raw projections as assigned profiles violates
`ClosedLineLedger.firstMatchOwnership`: the shared slope would be charged
twice. -/
...
/-- A one-line interface fixture retaining the required line-local profile sum.
It is not an actual RS row or an actual `(UNIF)` theorem. -/
...
/-- After correct deletion, the line-local distinct-slope budget and
natural-scale sum are both one; no raw C7 term is added. -/
