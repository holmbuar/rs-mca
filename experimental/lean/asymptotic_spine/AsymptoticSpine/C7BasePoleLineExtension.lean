...
This module is the final local composition step for the base-pole
constant-coefficient class.  The input line already contains the paid C1--C6
profiles on one received line.  Its flattened assigned-slope list is used as
the literal first-match deletion set.  The rooted C7 producer appends one unit
profile for every surviving constant-coefficient slope.

The result preserves duplicate-free first-match ownership and proves the exact
line-local telescopes

`combined budget = prior budget + number of C7 survivors`,

`combined natural sum = prior natural sum + number of C7 survivors`.
...
/-- Assigned slope list already carried by a closed C1--C6 line. -/
...
/-- The earlier line's assigned slopes are duplicate-free by its own ledger
ownership field. -/
...
theorem c7ListSum_append : forall xs ys : List Nat,
...
/-- Append the actual post-C1--C6 C7 singleton profiles to an already-closed
...
/-- The combined ray-budget sum is the previous line-local sum plus one unit
for every surviving C7 slope. -/
