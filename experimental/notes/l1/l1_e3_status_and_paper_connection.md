...
3. **Pencil / `s_k` conditions:** `X^ell - t*Gamma - b_k(t) = g_k(h_k-t s_k)` is exact (the `F_k` are
   ...
   consequence of `sum h_k q_k=0` and does not cut `Syz` (`pencil_cut_test.sage`). `l1_e3_formalization_run.md`.
...
The formalization skeleton is stored at `../../lean/l1_e3_level_set/`. It contains the
coset-level statement and partition backbone, while the main inequality remains an annotated
`sorry`. The sharpened problem statements are stored as `../../l1_e3_obligation.md` and
`../../l1_e3_obligation_v2.md`.
