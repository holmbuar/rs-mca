...
argument or formalization work on the clean sub-lemma.
...
is exactly what pulls `E_3` down to `<= ell-2`. The formalization obligation carries this hypothesis.

**Formalization obligation.** The KEY LEMMA (with the proven upper half and the `dim Syz <= K` crux)
is abstracted as pure finite-field algebra in `experimental/l1_e3_obligation.md`,
