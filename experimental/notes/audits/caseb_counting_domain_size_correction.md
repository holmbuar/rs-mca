...
The upstream PR supplied a Python verifier/certificate packet using exact
integers for every acceptance-critical comparison. It replays the three bad
calibrations, proves the universal target impossibility, checks both corrected
fixed-density families, proves the primality and exact half-rate degree of the
multiplicative rows, composes the exact prefix floor with theorem (4.2), checks
all six target crossings, gates the elementary \(p\ge61\) asymptotic family,
checks the two exact counting gates, pins the three source files, and rejects
nine semantic mutations. Every true-domain binomial is independently
reconstructed from Legendre prime exponents; every wrong-universe binomial is
independently replayed by the multiplicative recurrence.

This curated integration keeps the audit note, but intentionally does not
import the Python replay packet.
