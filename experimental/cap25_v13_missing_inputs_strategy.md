...
4. **Base of the recursion.** The fully stabilized fiber is $z=0$: subsets with vanishing first $w$ power sums — weight-$m$ $\{0,1\}$-words in the cyclic code with zeros $\zeta,\dots,\zeta^w$ (a BCH-dual weight-enumerator problem). Everything funnels here.

The output of Q2, if it goes through: **worst-case flatness reduces to flatness of the null fiber $N_w(0)$**, with all other heavy candidates charged to quotient rungs. That is a single, concrete, symmetric object.
...
1. *Max-fiber tables.* For small $n$ (say $n\le 40$, all divisors, several $(m,w)$), enumerate $N_w(z)$ for all $z$: test mode-at-null, measure $\max/\mathrm{avg}$ against $\binom Nm/|\mathbb B|^w$, and locate where the heavy fibers sit relative to the twist orbits.
...
| (Q-fin) mode-at-null | open conjecture | finite pair stays conditional; reposition closing as asymptotic theorem + certified-computation program |

**Bottom line.** Both missing inputs are shadows of one object — flatness of split-locator counts against affine conditions at the entropy–subfield critical density — with the quotient/graded strata as the (conjecturally unique) extremizers. The asymptotic forms are attackable now along three mutually reinforcing routes whose first rungs (A0, A.1, A.2, Q1, the audits) are provable with the machinery already in v12/v13. The finite adjacent pair is *not* an analytic problem: it is an extremality problem, and the strategy above names the exact statement (mode-at-null / exchange-compression) whose proof — or whose small-$n$ refutation — decides it.
