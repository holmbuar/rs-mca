...
rigid: Paper D gives a prize-facing upper cap on the MCA threshold, conditional
on the Crites--Stewart list-to-agreement import, while Paper A and the list
lower bounds rule out any no-slack capacity theorem.  The positive side is not
yet a theorem; it should be organized around two local-limit problems, L1
arbitrary-word locator fibers and M1 all-line residue packing, plus
extension-line MCA, the Crites--Stewart import audit, and exact protocol-ledger
rewrites.
...
result is Paper D's universal cap.  Conditional on the direct Crites--Stewart
conversion and the stated dyadic-divisor hypotheses,
\[
...
Paper D is the most prize-relevant negative theorem because it converts a list
phenomenon into CA and then MCA at the exact \(2^{-128}\)-style threshold.  Its
load-bearing import is the Crites--Stewart list-to-agreement conversion.  The
import must be audited, but the use is shallow: integer-radius compatibility,
the augmented-code list conclusion, normalization of \(\epsca\), domain/coset
coverage, and the CA-to-MCA monotonicity step.
...
\item \textbf{Crites--Stewart import audit.}  Paper D's universal cap is
load-bearing on the direct conversion.  The integer-radius condition,
augmented-code list conclusion, normalization of \(\epsca\), and domain/coset
hypotheses should be independently checked.
...
\item Keep the Crites--Stewart import marked \textsc{Conditional} until audited.
Paper D's cap is powerful enough to guide threshold claims, but the conversion
is load-bearing.
\end{enumerate}
...
\[
  ...
  1-\rho-\eta.}
\]
The theorem-backed negative side gives a conditional universal cap at constant
gap \(2^{-9}\) for \(\rho=1/2,1/4,1/8\) and \(2^{-10}\) for \(\rho=1/16\).  The
...
\textsc{Conditional} strict264 gate.}
...
corrected Paper B local-limit conjecture.  They isolate the exact checks that a
strict264 certificate and a matching 265 upper bound must satisfy.
...
For the current \(n=512,k=256\) row, the proposed next-step upper bound
\(\operatorname{LD}_{\rm sw}(C,265)\le6\) is a statement about degree
\(247\) split locators with \(t=9\).  Proposition~\ref{prop:fixed-locator-unique-slope}
reduces the finite all-line search to a duplicate-aware image problem for the
rational map \(T\mapsto z_T\), plus the consistency and noncontainment tests.
\end{remark}
...
For \(\qline=17^{32}\), the special case
\[
  B(264)\ge7,
  \qquad
  B(265)\le6
\]
pins the finite row to an unsafe/safe transition between
\[
  \delta_{265}=247/512
  \qquad\text{and}\qquad
  \delta_{264}=31/64.
