...
\title{Universal Field-Size Caps, Two-Sided Sandwiches, and Certificate Grammar\\
for Mutual Correlated Agreement on Smooth Reed--Solomon Domains}
...
\paragraph{Role of this manuscript.}
This is the main submission document for the present RS--MCA package.  The
companion papers and notes have distinct roles: the no-slack and slack papers
identify the obstruction mechanisms and reserve ledgers, the protocol paper
explains how those ledgers enter SNARK soundness, and the short
\emph{towards-prize} note extracts the current execution plan.  The final
mathematical claims intended for submission are consolidated here: the
field-size-universal unsafe cap, the safe-side pincer, the deployed-row
two-sided intervals, the model-transfer statements, and the finite certificate
grammar.  In particular, whenever a public row, scanner output, or companion
note cites the cap or deployed threshold package, this paper is the authority
for the hypotheses, endpoint conventions, denominators, and proof status.
